...
fun ItemsListItem(item: Item) {
    ...
    if(loaded){
        ...
        {
            Column {
                Text(text = item.itemName, style = typography.titleMedium, modifier = Modifier.padding(top = 16.dp, start = 16.dp))
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .wrapContentHeight()
